...
{
  ui->probListWidget->clear();
  ui->bigramTableWidget->clear();
  ui->bigramTableWidget->setColumnCount(2);
  ui->bigramTextLine->setText(ui->bigramTextLine->text().toUpper());
  QMap<QString, int> data = BigrammFreqAnalysis::calculate(ui->bigramTextLine->text(), ui->bigrammSize->value());
  ...
  ui->bigramTableWidget->sortItems(1, Qt::DescendingOrder);
  auto splittedString = ui->bigramTextLine->text().split(";");
  QString propstring("");
  QString tmp("");
  int count = 0;
  for (auto k = 0; k < ui->bigrammSize->value(); k++){
      for (auto i = k; i < ui->bigrammSize->value(); i++) {
          propstring = QString("p(") + QString::number(k) + QString(":") + QString::number(i) + QString(")=");
          for (auto j = 0; j < ui->bigrammSize->value(); j++) {
              tmp = QString(splittedString[j][k]) + QString(splittedString[j][i]);
              count += data[tmp];
            }
          ui->probListWidget->addItem(propstring + QString::number(count));
          count = 0;
        }
    }
}
